...
func ImportRoutes(path string) (int, error) {
	...
	for _, r := range routes {
		if err := upsertRouteTx(tx, r); err != nil {
			return 0, err
		}
	}
...
func ExportRoutes(path string) error {
	...
	w := csv.NewWriter(f)
	w.Comma = '\t'
	if err := w.Write(routeHeader); err != nil {
		return err
	}
	for rows.Next() {
		var r RouteRecord
		if err := rows.Scan(&r.FromSite, &r.ToSite, &r.Name, &r.Confidence, &r.Notes, &r.ExpectedDistanceMinKM, &r.ExpectedDistanceMaxKM, &r.ExpectedDurationMinMin, &r.ExpectedDurationMaxMin); err != nil {
			return err
		}
		rec := []string{
			r.FromSite,
...
func DeleteRoute(fromSite, toSite string) error {
	...
	if n == 0 {
		return fmt.Errorf("route not found: %s -> %s", fromSite, toSite)
	}
	return nil
}

func upsertRouteTx(exec interface {
	...
	if name == "" {
		name = fmt.Sprintf("%s -> %s", r.FromSite, r.ToSite)
	}
...
func siteID(q interface{ QueryRow(string, ...any) *sql.Row }, name string) (int64, error) {
	...
	if err := q.QueryRow(`SELECT id FROM sites WHERE name=?`, name).Scan(&id); err != nil {
		if err == sql.ErrNoRows {
			return 0, fmt.Errorf("unknown site %q", name)
		}
		return 0, err
	}
	return id, nil
...
func parseRoutesFile(path string) ([]RouteRecord, error) {
	...
	for _, c := range req {
		if _, ok := idx[c]; !ok {
			return nil, fmt.Errorf("%s: missing required column: %s", path, strings.Title(c))
		}
	}
	...
	for ln, row := range rows[1:] {
		...
		to := get(row, "to")
		if from == "" {
			return nil, fmt.Errorf("%s: row %d: column From: missing value", path, rowNum)
		}
		if to == "" {
			return nil, fmt.Errorf("%s: row %d: column To: missing value", path, rowNum)
		}
